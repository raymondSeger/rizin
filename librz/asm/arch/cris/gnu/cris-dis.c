...
enum cris_disass_family { cris_dis_v0_v10,
...
cris_parse_disassembler_options(disassemble_info *info,
	...
	/* Default true.  */
	disdata->trace_case = (!info->disassembler_options || (strcmp(info->disassembler_options, "nocase") != 0));
...
get_opcode_entry(unsigned int insn,
	...
	if (!opc_table) {
		...
		dip_prefixes = calloc(65536, sizeof(dip_prefixes[0]));
		if (!dip_prefixes) {
			return NULL;
		}

		bdapq_m1_prefixes = calloc(65536, sizeof(bdapq_m1_prefixes[0]));
		if (!bdapq_m1_prefixes) {
			return NULL;
		}

		bdapq_m2_prefixes = calloc(65536, sizeof(bdapq_m2_prefixes[0]));
		if (!bdapq_m2_prefixes) {
			return NULL;
		}

		bdapq_m4_prefixes = calloc(65536, sizeof(bdapq_m4_prefixes[0]));
		if (!bdapq_m4_prefixes) {
			return NULL;
		}

		rest_prefixes = calloc(65536, sizeof(rest_prefixes[0]));
		if (!rest_prefixes) {
			return NULL;
		}
	}
	...
	if (prefix_insn != NO_CRIS_PREFIX) {
		const struct cris_opcode *popcodep = (opc_table[prefix_insn] != NULL
...
print_flags(struct cris_disasm_data *disdata, unsigned int insn, char *cp) {
	...
	const char *fnames = disdata->distype == cris_dis_v32 ? v32_fnames : v8_fnames;
	...
	for (; *s; s++) {
	   ...
	   handled by setting it to NULL.  */
		...
		case 'n': {
			...
			unsigned long long number = (buffer[2] + buffer[3] * 256 + buffer[4] * 65536 + buffer[5] * 0x1000000 + addr);
		...
		case 's':
	       ...
	       number, where the size can depend on different things.  */
				long number;
				int signedp = ((*cs == 'z' && (insn & 0x20)) || opcodep->match == BDAP_QUICK_OPCODE);
				int nbytes;
		   ...
		   below.  */
		     ...
		     special registers.  */
				...
				case 4:
					number = buffer[2] + buffer[3] * 256 + buffer[4] * 65536 + buffer[5] * 0x1000000;
					break;
	       ...
	       prefixed) memory operand.  */
				if (info->insn_type != dis_nonbranch) {
					int mode_size = 1 << ((insn >> 4) & (opcodep->args[0] == 'z' ? 1 : 3));
					int size;
		   ...
		   as a side-effect address mode.  */
					&& ((insn & 0x400) == 0 || prefix_opcodep->match != DIP_OPCODE)) {
					if (insn & 0x400) {
						tp = format_reg(disdata, insn & 15, tp, with_reg_prefix);
						*tp++ = '=';
					}
		   ...
		   address-mode syntax is output.  */
			   ...
			   but an address.  */
							unsigned long number = prefix_buffer[2] + prefix_buffer[3] * 256 + prefix_buffer[4] * 65536 + prefix_buffer[5] * 0x1000000;
			   ...
			   In this case, target is zero.  */
							info->flags |= (CRIS_DIS_FLAG_MEM_TARGET2_IS_REG | CRIS_DIS_FLAG_MEM_TARGET2_MEM);
					...
					case BIAP_OPCODE:
						...
						*tp++ = mode_char[(prefix_insn >> 4) & 3];
						...
						info->flags |= (CRIS_DIS_FLAG_MEM_TARGET2_IS_REG | CRIS_DIS_FLAG_MEM_TARGET_IS_REG
		       ...
		       "r-s".  */
						...
						if ((prefix_insn & 0x400) && (prefix_insn & 15) == 15) {
							...
							case 4:
								number = prefix_buffer[2] + prefix_buffer[3] * 256 + prefix_buffer[4] * 65536 + prefix_buffer[5] * 0x1000000;
								break;
			   ...
			   address.  */
			       ...
			       bytes.  */
							...
							*tp++ = mode_char[(prefix_insn >> 4) & 3];
							...
							info->flags |= (CRIS_DIS_FLAG_MEM_TARGET2_IS_REG | CRIS_DIS_FLAG_MEM_TARGET2_MEM | CRIS_DIS_FLAG_MEM_TARGET_IS_REG
